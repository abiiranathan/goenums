...
static void state_create_or_alter_type(char c, enum State* state, char* cur_stmt, FILE* sql_file,
  ...
  if (ci_strstr(cur_stmt, "CREATE TYPE ") || ci_strstr(cur_stmt, "ALTER TYPE ")) {
    *state = INSIDE_ENUM_VALUES;
    ...
    // copy the type name to the current_enum including the null terminator
    strncpy(cur_enum->name, type_name, sizeof(cur_enum->name));
    ...
    switch (alterState) {
      case ADD: {
        ...
        for (int i = 0; i < current_enum->num_values; i++) {
          ...
          if (!duplicate) {
            strcpy(existing_enum->values[existing_enum->num_values++], current_enum->values[i]);
          }
        }
      } break;
      case RENAME: {
        ...
        int n = ci_sscanf(cur_stmt, "ALTER TYPE %*s RENAME VALUE %s TO %s", old_name, new_name);
        if (n != 2) {
      ...
      case DROP: {
        ...
        int n = ci_sscanf(cur_stmt, "ALTER TYPE %*s DROP VALUE %s", value_to_drop);
        if (n != 1) {
